...
const Page = (props: { params: { id: string } }) => {
  ...
  const {
    data: singlebudget,
    status: singleBudgetStatus,
    isPending: isSingleBudgetsPending,
  } = useQuery({
    queryKey: ['allBudgetCategories'],
  ...
  const monthlyBudget = budgets.sort((a, b) =>
    a.uid === params.id ? -1 : b.uid === params.id ? 1 : 0,
  );
  ...
  const {
    ...
    queryFn: () =>
      getBudgetDistributionApi(
        ...
        selectedUid ? selectedUid : monthlyBudget[0].uid,
      ),
  ...
  const [selectedMonth, setSelectedMonth] = useState(
    monthlyBudget.length > 0
      ? monthlyBudget[0]
      : { name: 'No Budget', percentage: 8, color: '#FF6384' },
  );
      ...
      >
        ...
        <div className=" px-[24px] gap-[16px] w-full border-b-1 border-b-[#F7F7F9] py-[8.5px]  flex justify-center">
          <button
            ...
            className=" border-[0.4px] text-[14px] items-center border-[#EFEFF0] w-full  bg-[#F7F7F9] rounded-[8px] flex p-[8px]  justify-between"
          >
        ...
        <div className=" w-full mt-[31px]  flex justify-center">
          ...
          {distributionStatus === 'error' && (
            <p>Failed to load budget data. Please try again.</p>
          )}
            ...
            <>
              {
                ...
                budgetDistributionData.distributions &&
                budgetDistributionData.distributions.length > 0 ? (
              ...
              <ul className="flex h-[396px] overflow-y-scroll flex-col">
                {monthlyBudget.map((month: any, index: number) => (
                  <div
                    ...
                    className={`py-[16px] cursor-pointer font-[500] border-b text-start px-[8px] ${
                      index === monthlyBudget.length - 1 ? '' : 'border-b-1'
                    } border-b-[#EFEFF0]`}
                  >
                    {month.name}
                  </div>
              ...
              <div className="flex py-[24px] flex-wrap ">
                {categories.map((category) => (
                  <button
                    ...
                    className={`mr-[16px] mb-[16px] text-[14px] p-[12px] rounded-lg transition-colors duration-200 ${
                      selectedCategories.includes(category.uid)
