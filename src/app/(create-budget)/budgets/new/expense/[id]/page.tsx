...
const Page = (props: { params: { id: string } }) => {
  ...
  const { getLastBudget, previousBudget, budgets, allCategories } = useBudgetStore(
    (state) => ({
  ...
  const createSubCategoryMutation = useMutation({
    mutationFn: (data: any) =>
      CreateSubCategoryApi(data.token, data.budgetCategoryId, data.name),
    onSuccess: (data: any) => {
      if (data?.success) {
        // console.log(data)
        return data;
  ...
  useEffect(() => {
    if (
      selectedBudget &&
      (!selectedBudget.subAllocations || selectedBudget.subAllocations.length === 0)
    ) {
  ...
  const handleAllocationChange = (
    ...
    const updatedSubAllocations = selectedBudget.subAllocations.map(
      (subAllocation: ISubAllocation, i: number) => {
        if (i === index) {
          return {
            ...subAllocation,
    ...
    if (selectedBudget && !hasSelectedBudget.current) {
      ...
      if (allDisplayedBudgets.length > 0) {
        ...
        const matchingBudget = allDisplayedBudgets.find(
          (budget) => budget?.budgetCategory?.uid === selectedBudget?.budgetCategory?.uid,
        );
  ...
  useEffect(() => {
    if (
      ...
      lastBudget?.allocations &&
      lastBudget?.allocations?.length > 0
    ) {
  ...
  const handleSaveCategory = async () => {
    ...
    if (totalSubAllocationAmount > (selectedBudget?.amount || 0)) {
      ...
      const updatedBudgets = allDisplayedBudgets.some(
        (budget) => budget?.budgetCategory?.uid === newData?.budgetCategory?.uid,
      )
        ? allDisplayedBudgets.map((budget) =>
            budget?.budgetCategory?.uid === newData?.budgetCategory?.uid
              ? newData
              : budget,
          )
      ...
      try {
        const percentage =
          Math.round((selectedBudget.amount / budgetStats?.income) * 100 * 100) / 100;
  ...
  return (
    ...
    >
      <Header
        link={`/budgets/new/income/${lastBudget?.id ?? ''}`}
      ...
      <div className="mt-[39.5px]  px-[24px] w-full">
        ...
        <div className=" my-[16px]  w-full">
          <div className=" w-full flex justify-between">
            ...
            <button
              onClick={() => setShowNewBudgetCategory(true)}
              className=" bg-[#EFEFF0] font-[500] text-[12px] rounded-[32px] py-[4px] px-[8px] flex gap-[4px] items-center "
            >
              {' '}
      ...
      <div
        className={` bg-[#F7F7F9]  grid ${budgetCategoriesArray?.length == 0 ? 'grid-cols-1' : 'grid-cols-1'}  gap-[12px] overflow-y-scroll overflow-x-hidden min-h-[322px] py-[16px] px-[24px] mb-[100px] `}
      >
        <div
          className={`grid ${fetchStatus === 'pending' || budgetCategoriesArray.length === 0 ? 'grid-cols-1' : 'grid-cols-2'} gap-[16px]`}
        >
          {fetchStatus === 'pending' ? (
            // Loading State
              ...
              {budgetCategoriesArray.length === 0 ? (
                ...
                <>
                  ...
                  {budgetCategoriesArray.map((budget: any) => {
                    ...
                    const allocation = lastBudget?.allocations?.find((allocation: any) =>
                      allocation?.budgetCategory?.uid
                        ? allocation?.budgetCategory?.uid === budget.uid
                        : allocation?.budgetCategory === budget.uid,
                    );
                    ...
                    return (
                      <ul
                        key={budget.uid}
                        className="budget-list grid grid-cols-1 gap-[16px] w-full"
                      >
                        <li
            ...
            footer={
              <div className="w-full grid gap-y-[16px]">
                <button
                  ...
                  className={` btn w-full rounded-[32px] px-[28px] py-[14px]  ${createSubCategoryMutation.isPending ? 'bg-[#434343]' : ' bg-black '} text-[#FAFAFA] flex items-center justify-center gap-[8px] font-[500] `}
                >
                  {' '}
                ...
                <button
                  onClick={() => handleDeleteOfCategory('111')}
                  className="btn w-full text-[#F5365C] rounded-[32px] px-[28px] py-[14px] bg-[#FBEDEF] flex items-center justify-center gap-[8px] font-[500]"
                >
                  Delete category
                </button>
              </div>
