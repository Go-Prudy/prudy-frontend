...
const Page = () => {
  ...
  const ChangePasswordMutation = useMutation({
    mutationFn: (data: any) => changePasswordApi(authenticatedUser?.token ?? '', data),
    onSuccess: (result) => {
      console.log(result);
  ...
  return (
    <div className="bg-[#FAFAFA]  w-[100vw] max-w-[500px] h-screen">
      ...
      <div className="mt-[24px] flex text-[#2D2D2D] flex-col w-full px-[24px]">
        <div>
          <h1 className="mb-[24px] font-[500] leading-[28px] text-[#575757]">
            Change your passcode
          </h1>

          {/* Old Passcode */}
          <label
            className="flex flex-col p-[16px] bg-[#F7F7F9] rounded-[20px] border border-[#EFEFF0] gap-[8px]"
            htmlFor="oldPasscode"
          >
        ...
        <div>
          ...
          <label
            className="mt-[16px] flex flex-col p-[16px] bg-[#F7F7F9] rounded-[20px] border border-[#EFEFF0] gap-[8px]"
            htmlFor="confirmPasscode"
          >
            <h1 className="text-[12px] text-[#575757] leading-[16px]">
              Confirm new passcode
            </h1>
            <input
