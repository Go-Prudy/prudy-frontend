...
import {
  GetBudgetCategoriesAnalyticsApi,
  GetOverallBudgetAnalyticsApi,
} from '@/app/services/AnalyticsService';
import {
  BudgetVsActualSkeleton,
  TopExpensesSkeleton,
} from '@/components/Skelentons/AnalysisSkeleton';
...
export default function Page() {
  ...
  const [showSubscriptionRestriction, setShowSubscriptionRestriction] =
  ...
  const {
    data: GetOverallBudgetAnalyticsData = [],
    isLoading: GetOverallBudgetAnalyticIsLoading,
    error: GetOverallBudgetAnalyticError,
    isPending: GetOverallBudgetAnalyticIsPending,
  } = useQuery({
    queryKey: ['GetOverallBudgetAnalyticsData', selectedBudget?.uid],
    queryFn: () =>
      GetOverallBudgetAnalyticsApi(authenticatedUser?.token ?? '', selectedBudget.uid),
    enabled: !!authenticatedUser?.token && !!selectedBudget.uid,
    refetchOnWindowFocus: true, // This should be directly in the options object.
  });

  const {
    data: GetBudgetCategoriesAnalyticsApiData = [],
    isLoading: GetBudgetCaGetBudgetCategoriesAnalyticsApiIsPending,
  } = useQuery({
    queryKey: ['GetBudgetCategoriesAnalyticsApi', selectedBudget?.uid],
    queryFn: () =>
      GetBudgetCategoriesAnalyticsApi(authenticatedUser?.token ?? '', selectedBudget.uid),
    enabled: !!authenticatedUser?.token && !!selectedBudget.uid,
    refetchOnWindowFocus: true, // This should be directly in the options object.
  });

  const { data: getSingleBudgetApiData = [], isLoading: getSingleBudgetApiIsPending } =
    useQuery({
  ...
  const bestPerformingCategory =
    GetOverallBudgetAnalyticsData?.bestPerformingCategory || {};
  const worstPerformingCategory =
    GetOverallBudgetAnalyticsData?.worstPerformingCategory || {};
    ...
    <>
      {showSubscriptionRestriction ? (
        <SubscriptionRestriction
          mainText="Do a lot more with GoPrudy"
          setShowSubscriptionRestriction={setShowSubscriptionRestriction}
          showSubscriptionRestriction={showSubscriptionRestriction}
        />
      ) : (
        <div className="bg-base-white w-[100vw] max-w-[500px] h-full">
          ...
          <div className="flex mb-[8px] w-full items-center py-[12px] px-[24px]  mt-[90px] gap-[12px]">
            <label className=" w-full bg-[#F7F7F9] px-[8px] py-[16px] rounded-[8px] border-[#EFEFF0] border flex flex-col gap-[8px] text-[12px] text-[#575757]">
              <button
                ...
                className="  text-[14px]  items-center border-[#EFEFF0] w-full  bg-[#F7F7F9] rounded-[8px] flex  justify-between"
              >
                {isPending ? (
                  <div className=" w-full   my-auto  flex  items-center">loading...</div>
                ) : (
                  <>
            ...
            {GetOverallBudgetAnalyticIsPending ? (
              ...
              <div className="w-full px-[24px]">
                <div className="p-[24px] rounded-t-[24px] border-[1px] border-[#EFEFF0]">
                  <h1 className="text-[#2D2D2D] mb-[16px] font-[500] leading-[16px]">
                    Budget vs Actual
                  </h1>

                  {/* Calculate the width percentages */}
                  {overall.breakdown.actualExpenses > 0 ? (
                    (() => {
                      const { totalBudgeted, actualExpenses } = overall.breakdown || {};

                      // Calculate the total for percentage calculation
                      const total = totalBudgeted + actualExpenses; // Sum of both values
                        ...
                        <>
                          ...
                          <div className="flex items-center mb-[12px]">
                            <h1
                              ...
                              style={{
                                width: `${adjustedBudgetPercentage}%`, // Use percentage based on total
                                minWidth: 'fit-content', // Ensure text fits fully before adjusting width
                          ...
                          {actualExpenses > 0 && (
                            <div className="flex items-center mb-[12px]">
                              <h1
                                ...
                                style={{
                                  width: `${adjustedActualPercentage}%`, // Use percentage based on total
                                  minWidth: 'fit-content', // Ensure text fits fully before adjusting width
                                  maxWidth: '100%', // Ensure it doesn't exceed container width
                                }}
                              >
                ...
                {overall.breakdown.actualExpenses > 0 ? (
                  <div className="rounded-b-[24px] pb-[24px] pt-[16px] bg-[#F3F0FA] px-[24px]">
                    ...
                    >
                      See categories breakdown{' '}
                      <span>
                        <BsChevronRight className="text-[#888888]" size={24} />
                      </span>
                    </button>
                  </div>
                ) : (
            ...
            {GetOverallBudgetAnalyticIsPending ? (
              ...
              <div className="w-full px-[24px]">
                <div className="p-[24px] w-full rounded-t-[24px] border-[1px] border-[#EFEFF0]">
                  ...
                  {GetOverallBudgetAnalyticsData?.topExpenses?.categories.length > 0 ? (
                    <div className="w-full flex flex-col gap-[12px]">
                      {GetOverallBudgetAnalyticsData?.topExpenses?.categories.map(
                        (expense: any) => (
                          <div
                            key={expense.uid}
            ...
            {GetOverallBudgetAnalyticIsPending ? (
              ...
              <div className="px-[24px]">
                <div className="p-[24px] rounded-t-[24px] border-[1px] border-[#EFEFF0]">
                  ...
                  {/* Calculate the width percentages */}
                  {(() => {
                    const { totalBudgeted, actualExpenses } =
                      bestPerformingCategory?.breakdown || {};

                    // Calculate the total for percentage calculation
                    const total = totalBudgeted + actualExpenses; // Ensure total is the sum of both values

                    // Calculate the adjusted percentages based on the total
                    const adjustedBudgetPercentage =
                      total > 0 ? (totalBudgeted / total) * 100 : 0;
                    const adjustedActualPercentage =
                      total > 0 ? (actualExpenses / total) * 100 : 0;
                    ...
                    return (
                      <div className=" p-[16px] bg-[#F7F7F9] border-[1px] border-[#EFEFF0] rounded-[16px] ">
                        {actualExpenses ? (
                          <>
                            <h1 className="mb-[10px] text-[14px] font-[500] ">
                              {bestPerformingCategory?.breakdown?.title}
                            </h1>
                            ...
                            <h1
                              ...
                              style={{
                                width: `${adjustedBudgetPercentage}%`,
                            ...
                            <h1
                              ...
                              style={{
                                width: `${adjustedActualPercentage}%`,
                                minWidth: 'fit-content',
                                maxWidth: '100%',
                              }}
                            >
                ...
                {bestPerformingCategory?.breakdown?.actualExpenses ? (
                  <div className="rounded-b-[24px] pb-[24px] pt-[16px] bg-[#F3F0FA] px-[24px]">
                    <h1 className="text-[#2D2D2D] font-[500] leading-[18px]">
                      {bestPerformingCategory?.remark?.title}
                    </h1>
                    <h1 className="mt-[8px] text-[14px] leading-[18px]">
                      {bestPerformingCategory?.remark?.description}
                    </h1>
                  </div>
                ) : (
                  <EmptyState />
            ...
            {GetOverallBudgetAnalyticIsPending ? (
              ...
              <div className="px-[24px]">
                <div className="p-[24px] rounded-t-[24px] border-[1px] border-[#EFEFF0]">
                  ...
                  <div className="p-[16px] bg-[#F7F7F9] border-[1px] border-[#EFEFF0] rounded-[16px]">
                    {worstPerformingCategory?.breakdown?.actualExpenses ? (
                      <>
                        {' '}
                        <h1 className="text-[#2D2D2D] mb-[16px] font-[500] leading-[16px]">
                          {worstPerformingCategory?.breakdown?.title}
                        </h1>
                        {/* Calculate the width percentages */}
                        {(() => {
                          const { totalBudgeted, actualExpenses } =
                            worstPerformingCategory?.breakdown || {};
                          ...
                          const adjustedBudgetPercentage =
                            (totalBudgeted / maxValue) * 100;
                          const adjustedActualPercentage =
                            (actualExpenses / maxValue) * 100;
                          ...
                          return (
                            <>
                              ...
                              <div className="flex items-center mb-[12px]">
                                <h1
                                  className="bg-[#66C227] text-[#FFFFFF] mb-[12px] text-[10px] px-[10px] py-[6px] flex rounded-r-[8px] font-[700] leading-[16px] text-ellipsis whitespace-nowrap overflow-hidden"
                                  style={{
                                    width: `${adjustedBudgetPercentage}%`, // Use percentage based on maximum value
                                    minWidth: 'fit-content', // Ensure text fits fully before adjusting width
                                    maxWidth: '100%', // Ensure it doesn't exceed container width
                                  }}
                                >
                              ...
                              <div className="flex items-center mb-[12px]">
                                <h1
                                  className="bg-[#F89446] text-[#FFFFFF] mb-[12px] text-[10px] px-[10px] py-[6px] flex rounded-r-[8px] font-[700] leading-[16px] text-ellipsis whitespace-nowrap overflow-hidden"
                                  style={{
                                    width: `${adjustedActualPercentage}%`, // Use percentage based on maximum value
                                    minWidth: 'fit-content', // Ensure text fits fully before adjusting width
                                    maxWidth: '100%', // Ensure it doesn't exceed container width
                                  }}
                                >
          ...
          {showBudget && (
            <AnimatePresence>
              <motion.div
                ...
                className="h-[120vh] w-full max-w-[500px] z-[40] bottom-0 fixed bg-[#1c1c1c73]"
              >
                <BottomDrawer
                ...
                <div className="h-[86vh] mt-[32px]   overflow-y-auto ">
                  {GetBudgetCategoriesAnalyticsApiData.map((item: any, index: any) => {
                    ...
                    const differencePercentage = Math.min(
                      (Math.abs(difference) / totalBudgeted) * 100,
                      100,
                    ); // Cap at 100%
                    ...
                    return (
                      <div
                        key={index}
                        className="mb-[24px] bg-[#F7F7F9] border border-[#EFEFF0] rounded-[24px] p-[24px]"
                      >
                        {/* Display the type of expense */}
                        <h2 className="text-[14px] text-[#2D2D2D] font-[500] mb-[4px]">
                          {title}
                        </h2>
                        ...
                        <h1
                          ...
                          style={{
                            width:
                              adjustedBudgetPercentage < 30
                                ? 'fit-content'
                                : `${adjustedBudgetPercentage}%`, // Use 'fit-content' for small percentages
                        ...
                        <h1
                          ...
                          style={{
                            width:
                              adjustedActualPercentage < 40
                                ? 'fit-content'
                                : `${adjustedActualPercentage}%`, // Use calculated width based on the actual expenses
                            maxWidth: '100%', // Ensure the bar does not exceed the parent div
                          }}
                        >
                          <span className="text-[10px] font-[400]">
                        ...
                        {/* Remark section */}
                        <div>
                          <h3 className="text-[12px] text-[#2D2D2D] font-[500] mb-[4px]">
                            {item.remark.title}
                          </h3>
          ...
          {showExpenseBreakDown && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.3 }}
              className="h-[100vh] w-full max-w-[500px] z-[40] bottom-0 fixed bg-[#1c1c1c73]"
            >
              <BottomDrawer
                label={`Top expenses`}
                back={false}
...
const EmptyState = () => (
  <div className="rounded-b-[24px] pb-[24px] pt-[16px] bg-[#F3F0FA] px-[24px]">
    <h1 className="text-[#2D2D2D] font-[500] leading-[18px]">
      You’re yet to record any expenses
    </h1>
    <h1 className="mt-[8px] text-[14px] leading-[18px]">
      Go to Track to record your expenses and see your analytics clearly here.
    </h1>
  </div>
);
