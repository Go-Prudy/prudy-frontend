...
const Page = ({ params }: { params: { id: string } }) => {
  ...
  const removeSubAllocation = (index: number) => {
    const updatedAllocations: SubAllocation[] = selectedBudget.subAllocations.filter(
      (_: SubAllocation, i: number) => i !== index,
    );
  ...
  return (
    ...
    >
      <Header
        light={false}
        link="/budgets"
        title={singleBudgetData?.name || 'untitled'}
      />
      ...
      >
        ...
        <div
          className={` bg-[#F7F7F9]  grid ${singleBudgetData?.budgetCategories?.length < 2 || singleBudgetStatus ? 'grid-cols-1' : 'grid-cols-2'}  gap-[12px] overflow-y-scroll overflow-x-hidden max-h-[322px] py-[16px] px-[24px] w-full mb-[24px] `}
        >
          {singleBudgetStatus ? (
            <CircularProgress
              className=" mx-auto w-full mt-[3rem]"
              size="md"
              color="default"
            />
          ) : null}
          ...
          {singleBudgetData?.budgetCategories?.length == 0 ? (
            ...
            <>
              {singleBudgetData?.budgetCategories?.map(
                (budget: any, budgetIndex: number) => (
                  <div
                    key={budget.uid}
                  ...
                  >
                    ...
                    <h1 className="font-[500] text-[14px]">
                      ₦ {budget.amountLeft.toLocaleString()}{' '}
                      <span className="font-[400] text-[10px] text-[#828282]">left</span>
                    </h1>
                    <Progress
        ...
        <div className="  border-none    px-[24px] w-full ">
          <div
            onClick={() => navigation.push(`/budgets/${params.id}/distribution/`)}
            className="bg-[#F7F7F9] cursor-pointer rounded-[20px]  border border-[#EFEFF0] p-[16px] flex justify-between w-full"
          >
            <div className=" flex  gap-[12px] items-start">
              ...
              <div className=" ">
                ...
                <p className=" text-[12px] text-[#575757]">
                  See how your budget is distributed <br /> across all the categories
                </p>
              </div>
            </div>
          ...
          <div className=" flex justify-between w-full">
            <h1 className=" text-[#474747] my-[24px] text-[18px] font-[500] ">
              Collaborators
            </h1>

            {singleBudgetData?.collaborators?.length >= 2 && (
              <button className=" text-[#474747] my-[24px] flex  justify-center  text-[12px] font-[500] rounded-[32px] w-[130px] bg-[#ECFDDC]   gap-[.4px] items-center ">
                <BsPlus size={20} />{' '}
                <span className=" font-[500] ">Invite collaborator</span>
              </button>
            )}
          </div>
          ...
          <div className=" w-full pb-[36px]  flex gap-[16px]">
            <div
              className={` w-full grid ${singleBudgetData?.collaborators?.length <= 1 ? 'grid-cols-1' : 'grid-cols-2'} `}
            >
              {singleBudgetStatus ? (
                <CircularProgress
                  className="mx-auto w-full mt-[3rem]"
                  ...
                  {singleBudgetData?.collaborators?.length > 0 ? (
                    singleBudgetData?.collaborators?.map((item: any) => (
                      ...
                      >
                        ...
                        <h1 className="text-[#2D2D2D] text-base font-medium text-center">
                          {item.name}
                        </h1>
                        <h1 className="text-[#828282] rounded-[10px] bg-white text-sm px-[8px]  py-[2px] font-medium">
                          {item.isHost ? 'HOST' : 'GUEST'}
                        </h1>
                      </div>
                    ))
                  ) : (
                    <p className="text-center text-[#A3A3A3] mt-4">
            ...
            {singleBudgetData?.collaborators?.length <= 1 && (
              <div
                ...
                className="rounded-[20px] h-[172px] bg-[#F5FEED] items-center border-dashed border-[#66C227] border-2 p-[16px] flex flex-col justify-center gap-3 w-full"
              >
                <Image
                  src={add}
                  width={1000}
                      ...
                      <div className="flex items-start">
                        <div className="bg-white rounded-[8px] py-[4px] px-[8px] flex items-start gap-[8px]">
                          ...
                          <div className="relative inline-block">
                            <input
                              value={
                                isNaN(eachSubAllocation.amount)
                                  ? ''
