...
const Scanner: React.FC<{
  ...
  setManualData,
  setAddManualModal,
  setAddManualModalTitle,
  categoryId,
}) => {
  const [cameraFacingMode, setCameraFacingMode] = useState<'user' | 'environment'>(
    'environment',
  );
  const [stream, setStream] = useState<MediaStream | null>(null); // Add this new state
  ...
  // Modify the useEffect hook to handle cleanup
  useEffect(() => {
    ...
    // Only request camera permission if isCameraActive is true
    if (isCameraActive) {
      requestCameraPermission();
    }

    // Cleanup function to stop all tracks when component unmounts
    // or when isCameraActive becomes false
    return () => {
      if (stream) {
  ...
  }, [isCameraActive]); // Add isCameraActive as dependency
