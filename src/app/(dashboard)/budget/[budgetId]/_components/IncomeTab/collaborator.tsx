...
export default function CollaboratorItem({ picture, name, email, isHost }: Props) {
  return (
    <div className="bg-gray-100 rounded-2xl border border-gray-200 p-3 sm:p-4 pr-2 sm:pr-3 flex items-start gap-1 justify-between">
      <div className="flex items-center gap-3">
        {picture ? (
          ...
          <div className="bg-gray-200 w-8 sm:w-10 h-8 sm:h-10 rounded-full flex items-center justify-center text-gray-600">
            <BsPerson />
          </div>
        )}
        <div className="space-y-2">
          <p className="text-black-800 text-sm sm:text-base font-medium">{name}</p>
          {email && <p className="text-sm sm:text-base text-gray-400">{email}</p>}
        </div>
      </div>
