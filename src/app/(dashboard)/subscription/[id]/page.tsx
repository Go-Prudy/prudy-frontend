...
import {
  completeAddPaymentMethodApi,
  deletePaymentMethodApi,
  getBillingCycleApi,
  getBillingHistoryApi,
  getPaymentMethodsApi,
  initAddPaymentMethodApi,
...
const Page = (props0: {
  ...
  const [showSuccesfulCancelSubscription, setShowSuccesfulCancelSubscription] =
    useState(false);
  ...
  const {
    ...
    isPending: isGetBillingCyclePending,
    isError: isGetBillingCycleError,
  } = useQuery({
  ...
  const {
    ...
    isPending: isGetBillingHistoryPending,
    isError: isGetBillingCycleHistoryError,
    isFetching,
  } = useQuery({
    queryKey: ['getbillingHistory'],
    queryFn: () =>
      getBillingHistoryApi(authenticatedUser?.token ?? '', currentPage, limit),
    enabled: !!authenticatedUser?.token, // Only fetch if token exists
    refetchOnWindowFocus: false, // Prevent refetching on window focus
    refetchOnMount: false, // Prevent refetching on component mount
    refetchInterval: false, // Disable polling
  ...
  const {
    ...
    isPending: isGetPaymentMethodPending,
  ...
  const completeAddPaymentMethodMutation = useMutation({
    mutationFn: ({ token, paymentRef }: { token: string; paymentRef: string }) =>
      completeAddPaymentMethodApi(token, paymentRef),
    onSuccess: (data: any) => {
  ...
  const setDefaultPaymentMethodMutation = useMutation({
    mutationFn: ({ token, cardId }: { token: string; cardId: string }) =>
      setDefaultPaymentMethodApi(token, cardId),
  ...
  const deletePaymentMethodMutation = useMutation({
    mutationFn: ({ token, cardId }: { token: string; cardId: string }) =>
      deletePaymentMethodApi(token, cardId),
    onMutate: async ({ cardId }) => {
      await queryClient.cancelQueries({ queryKey: ['paymentMethods'] });
      const previousPaymentMethods = queryClient.getQueryData(['paymentMethods']);
      queryClient.setQueryData(['paymentMethods'], (old: any) =>
  ...
  const CustomRadio = (props: any) => {
    ...
    const { duration, header1, header2, header3, label, label2Text, labelText } = props;
          ...
          <div className=" bg-[#F7F7F9] mb-[28px] rounded-[12px]  flex  p-[4px] mt-[12px]  ">
            {tabs.map((tab: string, i) => (
              <button
                ...
                key={i}
                className={` w-full  rounded-[12px] py-[8px] items-center px-[21px] ${activeTab === tab ? ' bg-white' : ''}`}
              >
                {tab}
              </button>
            ))}
          ...
          <div className="flex px-[8px] items-center justify-between">
            ...
            {activeTab === 'Subscription plans' ? (
              <Button
                ...
                className=" bg-[#fff] border border-[#E7E7EA] font-[400] text-[14px] flex gap-[.8rem] justify-between rounded-[24px] px-[12px] py-[8px]  items-center"
              >
                Change plan
              </Button>
            ) : (
              <button
                ...
                className="text-[12px] cursor-pointer font-[400] text-[#D2303E]"
              >
                Cancel Subscription
              </button>
            )}
            ...
            <div>
                    ...
                    <div key={getSinglePalnData?.data?.uid} className="h-[242px] w-full ">
                      <CustomRadio
                        ...
                        header3={
                          getSinglePalnData?.data?.basePrice !== 0
                            ? getSinglePalnData?.data?.weeklyAmount &&
                              getSinglePalnData?.data?.weeklyAmount > 0
                        ...
                        label2Text={
                          getSinglePalnData?.data?.uid === params.id
                            ? 'Current plan'
                            : 'd'
                        }
                      ...
                      >
                        <ul className="flex flex-col gap-[8px] pl-[1.5rem] mt-[5px] list-disc">
                          {getSinglePalnData?.data?.benefits?.map(
                            (benefit: any, index: any) => <li key={index}>{benefit}</li>,
                          )}
                        </ul>
                      </CustomRadio>
                    </div>
              ...
              <div className="flex justify-between items-center mt-[40px]">
                ...
                <button
                  ...
                  className="bordr-[#E7E7EA] border rounded-[16px] p-[8px] text-[14px] text-[#575757] flex gap-[4px] items-center"
                >
                  <span>
                    <BsPlus size={20} />
                  </span>{' '}
                      ...
                      {getPaymentMethodData.map((item: any) => (
                        ...
                        >
                          <div className="flex gap-[12px] items-start">
                            ...
                            <div>
                              <p className="text-[12px] px-[8px] py-[2px] rounded-[12px] bg-white text-[#66C227]">
                            ...
                            {visibleTooltipUid === item.uid && (
                              <div className="absolute top-[60%] grid gap-[24px] place-content-center w-[109px] min-h-[72px]  right-[20px] transform translate-y-2 z-10 bg-white p-2 rounded-md shadow-xl">
                                <div className="flex text-[#575757] gap-[8px] flex-col">
                                  <button
                                    className="text-[12px] p-2  hover:bg-gray-100 rounded-md"
                                    onClick={() =>
                                      handleSetDefaultPaymentMethod(item.uid)
                                    }
                                  >
                                    Make Default
                                  </button>
                                  <button
                                    className="text-[12px] p-2  hover:bg-gray-100 rounded-md"
                                    onClick={() => handleDeletePaymentMethod(item.uid)}
                                  >
                                    Delete Card
                                  </button>
                                </div>
                ...
                <>
                  ...
                  <div>
                    <h1 className="text-[16px] font-[600]">
                      ₦{' '}
                      {billingCycleData?.amount
                        ? billingCycleData?.amount.toFixed(2)
                        : '0'}
                    </h1>
                  </div>
                </>
              ...
              {isGetBillingHistoryPending ? (
                ...
                <div className="flex flex-col gap-[16px]">
                  {billingHistoryData?.docs?.length > 0 ? (
                    billingHistoryData.docs.map(
                      (history: IBillingHistory, index: number) => (
          ...
          >
            <div className="bg-white flex flex-col rounded-[40px] items-center text-center w-full p-8">
              ...
              <p className="text-[#707170] leading-[24px]">
                You are about to cancel your subscription from Prudy. Are you sure you
              ...
              <div className="mt-[16px] p-[4px] bg-[#F7F7F9] rounded-[12px] mb-[20px] flex justify-center w-fit mx-auto">
                {allOptions.map((option: any) => (
                  <button
                    ...
                    className={`${option === selectedOption && 'rounded-[12px] text-white bg-[#66C227]'} p-[8px]`}
                    key={option}
                  >
                    {option}
                  </button>
