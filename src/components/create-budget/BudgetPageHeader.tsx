...
const BudgetPageHeader = ({ headerType, title }: BudgetPageHeaderProp) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={` fixed w-full ${!menuOpen ? 'left-[-500rem]' : 'left-0'} duration-100 transition-all ease-in  top-0 h-[100vh] z-50 `}
      >
        <div className="relative w-full">
          <div
            className="bg-white w-[295px] h-[100vh] overflow-y-scroll z-[150] relative py-[48px] px-[24px] transition-transform duration-300"
            style={{ transform: menuOpen ? 'translateX(0)' : 'translateX(-100%)' }}
          >
            <div className="flex items-start gap-[8px]">
              <div
                className={`flex items-center rounded-full ${!userData.profile.profilePhotoUrl ? 'p-1 border border-[#101010]' : 'p-0'}  `}
              >
                {userData.profile.profilePhotoUrl ? (
                  <Image
                    src={userData.profile.profilePhotoUrl}
            ...
            <ul className="flex mt-[40px] gap-[12px] flex-col justify-around ">
              <Link
                href="/budgets"
                className={`p-[12px] rounded-[12px] w-[247px] h-[48px] flex items-center py-[7px] gap-[8px] ${currentRoute === '/budgets' ? 'bg-[#ECFDDC] text-[#66C227]' : 'text-[#828282]'}`}
              >
              ...
              <Link
                href="/track"
                className={`p-[12px] rounded-[12px] w-[247px] h-[48px] flex items-center py-[7px] gap-[8px] ${currentRoute === '/track' ? 'bg-[#ECFDDC] text-[#66C227]' : 'text-[#828282]'}`}
              >
                {currentRoute === '/track' ? (
                  <Image
                    className="size-[24px]"
              ...
              <Link
                href="/analytics"
                className={`p-[12px] rounded-[12px] w-[247px] h-[48px] flex items-center py-[7px] gap-[8px] ${currentRoute === '/analytics' ? 'bg-[#ECFDDC] text-[#66C227]' : 'text-[#828282]'}`}
              >
              ...
              <Link
                href="/profile"
                className={`p-[12px] rounded-[12px] w-[247px] h-[48px] flex items-center py-[7px] gap-[8px] ${currentRoute === '/profile' ? 'bg-[#ECFDDC] text-[#66C227]' : 'text-[#828282]'}`}
              >
              ...
              <Link
                ...
                className={`p-[12px] rounded-[12px] w-[247px] h-[48px] flex items-center py-[7px] gap-[8px] ${currentRoute === '/settings/passcode' ? 'bg-[#ECFDDC] text-[#66C227]' : 'text-[#828282]'}`}
              >
                {currentRoute === '/settings/passcode' ? (
                  <Image
                    className="size-[24px]"
                    alt="profile"
                    src={setting1}
          ...
          <div
            className=" absolute z-[24] top-0 left-0 h-[100vh] bg-[#2828286e] backdrop-blur-3xl w-[500px] "
            onClick={toggleMenu}
          />
          {!toggleMenu ? (
            <Image
              className=" size-[48px] top-[24px] z-[50] absolute right-[24px] cursor-pointer"
              alt="close"
              width={1000}
              height={1000}
              src={close}
